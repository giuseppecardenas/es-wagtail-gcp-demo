...
def search(request):
    ...
    locale_param = request.GET.get('locale', None)
    locale = 'en-us' if not locale_param else locale_param
    ...
    if search_query:
        if 'elasticsearch' in settings.WAGTAILSEARCH_BACKENDS['default']['BACKEND']:
            ...
            # like this:
            if locale not in settings.ELASTICSEARCH_INDICES:
                search_results = Page.objects.none()
            else:
                backend = settings.ELASTICSEARCH_INDICES[locale]
                search_results = Page.objects.live().search(search_query, backend=backend)
        else:
